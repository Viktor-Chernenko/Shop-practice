...
$(document).ready(function () {
    ...
    var functions = {
        addEmployer: $(mainList.btn.addEmployer).on('click', function (e) {
            ...
            for(let i = 0; i < mainList.input.employer.name.length; i++) {
            ...
            for(let i = 0; i < mainList.input.employer.position.length; i++) {
                $((mainList.input.employer.position)[i]).on('blur', function () {
                    if(parseInt($(this).val()) && $(this).val() !== '') {
                        $('.pop-up').addClass('pop-up_active');
        ...
        result: $(mainList.btn.result).on('click', function (e) {
            ...
            if($('.mainList__employer').length <= $('.mainList-employer').length) {
                ...
                for(let i = 1; i < $('.mainList-employer').length; i++) {
            ...
            if(mainList.input.employer.name.val().length > 0 || mainList.input.employer.position.val().length > 0) {
                for(let i = 0; i < mainList.input.employer.box.length; i++) {
                    mainList.employers[$((mainList.input.employer.box)[i]).find(mainList.input.employer.name).val()] = $((mainList.input.employer.box)[i]).find(mainList.input.employer.position).val();
                }

                for(let k = 0; k < $('.mainList__employer').length; k++) {
                    let text = Object.keys(mainList.employers)[k];
